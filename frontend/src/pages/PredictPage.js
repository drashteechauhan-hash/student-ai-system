...
const init = {
  student_name: '',  // 👈 YE ADD KARO
  age: 19,
  gender: 'Female',
  age:19,gender:'Female',education_level:'Undergraduate',stream:'Engineering',
...
export default function PredictPage() {
  ...
  const perfColor = result?.performance_category==='High'?'var(--green)':result?.performance_category==='Medium'?'var(--yellow)':'var(--red)';
  const riskColor = result?.risk_level?.includes('High')?'var(--red)':result?.risk_level?.includes('Moderate')?'var(--yellow)':'var(--green)';
  ...
  />
            ...
            <div className="form-section">
              ...
              <div className="form-grid">
                ...
                <F label="Screen Time (non-study)" name="screen_time" value={form.screen_time} options={['Less than 1 hour','1–3 hours','3–5 hours','More than 5 hours']} onChange={set}/>
            ...
            <div className="form-section">
              ...
              <div className="form-grid">
                <R label="Stress Level (1=Low, 5=High)" name="stress_level" value={form.stress_level} min={1} max={5} onChange={set}/>
                <R label="Motivation Level (1–5)" name="motivation_level" value={form.motivation_level} min={1} max={5} onChange={set}/>
                <R label="Exam Anxiety (1=Low, 5=High)" name="exam_anxiety" value={form.exam_anxiety} min={1} max={5} onChange={set}/>
          ...
          <button className="btn btn-primary btn-full" onClick={predict} disabled={loading}>
            {loading?<><div className="spinner"/>Analysing...</>:'Predict Performance →'}
          </button>
        </div>
        ...
        <div style={{position:'sticky',top:24}}>
          {!result ? (
            <div className="card result-empty">
              ...
              <div style={{fontSize:15,fontWeight:600,marginBottom:6,color:'var(--text)'}}>No prediction yet</div>
              <div style={{fontSize:13,color:'var(--text3)'}}>Fill in the form and click Predict</div>
            </div>
          ) : (
            <div className="card fade-in">
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                <span style={{fontWeight:600,fontSize:15}}>Result</span>
                <span style={{fontSize:11,color:'var(--green)',background:'#dcfce7',padding:'3px 8px',borderRadius:4,fontWeight:600}}>AI Analysed</span>
              </div>

              <div style={{textAlign:'center',padding:'20px 0',borderBottom:'1px solid var(--border)',marginBottom:16}}>
                <div style={{fontSize:12,color:'var(--text3)',marginBottom:4}}>Predicted Score</div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>
                {[{l:'Wellness',v:result.wellness_score},{l:'Engagement',v:result.engagement_score}].map(s=>(
                  <div key={s.l} style={{background:'var(--bg)',borderRadius:8,padding:'10px',textAlign:'center',border:'1px solid var(--border)'}}>
                    <div style={{fontSize:20,fontWeight:700,color:'var(--accent)'}}>{s.v}</div>
                  ...
                  {result.weak_areas.map((a,i)=>(
                    <div key={i} className="progress-wrap">
                      <div className="progress-header">
                        <span style={{fontSize:13,color:a.weak?'var(--red)':'var(--text)'}}>{a.weak?'⚠️ ':''}{a.area}</span>
              ...
              {tab==='radar' && (
                <ResponsiveContainer width="100%" height={200}>
                  <RadarChart data={radarData}>
                    ...
                    <PolarAngleAxis dataKey="s" tick={{fontSize:11,fill:'var(--text2)'}}/>
                    <PolarRadiusAxis angle={30} domain={[0,100]} tick={false}/>
                    <Radar name="Student" dataKey="v" stroke="var(--accent)" fill="var(--accent)" fillOpacity={0.2}/>
